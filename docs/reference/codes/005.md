...
```console
$ broot -V
INFO[0000] Installing a crate                            khulnasoft_version= env=darwin/arm64 package_name=crates.io/broot package_version=1.20.0 program=khulnasoft registry=local
...
INFO[0109] removing the install directory because the installation failed  khulnasoft_version= env=darwin/arm64 install_dir=/Users/shunsukesuzuki/.local/share/khulnasoftproj-khulnasoft/pkgs/cargo/crates.io/broot/1.20.0 package_name=crates.io/broot package_version=1.20.0 program=khulnasoft registry=local
ERRO[0109] install the package                           khulnasoft_version= env=darwin/arm64 error="cargo install: install a crate: exit status 101" package_name=crates.io/broot package_version=1.20.0 program=khulnasoft registry=local
FATA[0109] khulnasoft failed                                   khulnasoft_version= env=darwin/arm64 error="it failed to install some packages" program=khulnasoft
```

## What does this error mean?

khulnasoft runs `cargo install` command to install `cargo` type packages, but `cargo install` command fails.

## How to solve the error

Probably this error isn't the issue of khulnasoft.

Please run `cargo install` command and check if the issue can be reproduced without khulnasoft.
...
if you need help, please ask not khulnasoft maintainers but Rust community.
